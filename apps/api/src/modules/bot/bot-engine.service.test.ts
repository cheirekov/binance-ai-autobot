...
describe("bot-engine insufficient-balance helpers", () => {
  ...
  it("classifies grid guard pause skips as storm-eligible", () => {
    const helpers = service as unknown as {
      getSkipStormKey: (summary: string) => string | null;
    };

    const key = helpers.getSkipStormKey("Skip XRPUSDC: Grid guard paused BUY leg");
  ...
  it("uses gentler skip-storm trigger for grid guard pause loops", () => {
    const helpers = service as unknown as {
      deriveInfeasibleSymbolCooldown: (params: {
        state: BotState;
        symbol: string;
    ...
    const summary = "Skip XRPUSDC: Grid guard paused BUY leg";
    const now = Date.now();
    const oneRecentState: BotState = {
      ...defaultBotState(),
      decisions: [
