...
case "$ACTIVE_TICKET" in
  T-032)
    ...
    echo "Active ticket: $ACTIVE_TICKET"
    run_in_ci "corepack enable && pnpm install --no-frozen-lockfile && pnpm -C apps/api exec vitest run src/modules/bot/bot-engine.service.test.ts -t 'caution unwind|defensive grid-guard unwind'"
    ;;
  *)
