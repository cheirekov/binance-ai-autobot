...
## Problem statement
The newest fresh bundle (`autobot-feedback-20260329-150750.tgz`) shows the active blocker is now execution ordering under `T-031`: while `risk_state=CAUTION`, already-open managed symbols can still hit `Fee/edge filter` and return early before downside-control handling runs.
...
- A bounded `T-031` slice that bypasses fee-edge gating for already-open managed symbols during `DEFENSIVE` / loss-guard handling will reduce `CAUTION` churn without reopening `T-032` downside-control or `T-034` funding regressions.
...
- the next fresh bundle reflects lower managed-symbol fee-edge churn during `CAUTION` without reopening funding or downside-control regressions
