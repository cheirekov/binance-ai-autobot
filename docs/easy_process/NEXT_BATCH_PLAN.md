...
## Exact scope
Keep active development on `T-031`. Implement the next bounded `T-031` slice so `SPOT_GRID` candidate ranking follows the resolved execution lane instead of mostly favoring grid-style candidates. Preserve current `T-032` downside controls and `T-034` funding stability.
...
- the switch is documented in board/changelog/session/ticket-switch artifacts
...
Moves `Lane C — Strategy quality` by turning the current live blocker into an explicit `T-031` strategy batch instead of continuing lower-leverage `T-032` proof cycles.
