...
## Exact scope
Keep `T-031` active. Implement one bounded widening step for the family-level residual storm slice so repeated `Grid sell leg not actionable yet` skips across multiple home-quote dust symbols trigger faster and park longer as a cluster. Preserve the April 12 linked-support thaw, the broader April 2 deadlock recovery, April 9-10 residual mitigation, March 30-31 `T-032` downside-control behavior, and `T-034` funding stability.
...
## In scope
- widen the family-level residual storm window/threshold/cooldown in runtime code + tests
...
- the widened family-level residual storm slice lands with tests
- the next fresh bundle can show lower repeated residual-family churn and fewer short cooldown re-entries without reopening funding or downside-control regressions
