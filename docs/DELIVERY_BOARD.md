...
4. `T-034` Multi-quote execution policy v1 (adaptive quotes, fee-aware conversion edge).
5. `T-030` Universe filter-chain v2 (NFI-inspired staged filters + explainability).
6. `T-031` Regime engine v2 (RSI bull/bear + ADX dynamic thresholds).
7. `T-032` Exit manager v2 (supertrend/ATR + progressive ratchet).
8. `T-025` Adaptive confidence shadow model v1 (AI specialist lane; shadow-first).
9. `T-028` Compact Advanced settings UX (non-blocking; reduce Advanced page noise when risk-managed).
